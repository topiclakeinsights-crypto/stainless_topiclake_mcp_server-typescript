...
import { APIPromise } from 'stainless_topiclake_mcp_server/core/api-promise';

import util from 'node:util';
import StainlessTopiclakeMcpServer from 'stainless_topiclake_mcp_server';
import { APIUserAbortError } from 'stainless_topiclake_mcp_server';
...
describe('instantiate client', () => {
  ...
  describe('logging', () => {
    ...
    beforeEach(() => {
      process.env = { ...env };
      process.env['STAINLESS_TOPICLAKE_MCP_SERVER_LOG'] = undefined;
    });
    ...
    const forceAPIResponseForClient = async (client: StainlessTopiclakeMcpServer) => {
    ...
    test('default logLevel is warn', async () => {
      const client = new StainlessTopiclakeMcpServer({ apiKey: 'My API Key' });
    ...
    test('debug logs happen with debug env var', async () => {
      ...
      process.env['STAINLESS_TOPICLAKE_MCP_SERVER_LOG'] = 'debug';
      const client = new StainlessTopiclakeMcpServer({ logger: logger, apiKey: 'My API Key' });
    ...
    test('warn when env var level is invalid', async () => {
      ...
      process.env['STAINLESS_TOPICLAKE_MCP_SERVER_LOG'] = 'not a log level';
      const client = new StainlessTopiclakeMcpServer({ logger: logger, apiKey: 'My API Key' });
      expect(client.logLevel).toBe('warn');
      expect(warnMock).toHaveBeenCalledWith(
        'process.env[\'STAINLESS_TOPICLAKE_MCP_SERVER_LOG\'] was set to "not a log level", expected one of ["off","error","warn","info","debug"]',
      );
    });
  ...
  describe('baseUrl', () => {
    ...
    afterEach(() => {
      process.env['STAINLESS_TOPICLAKE_MCP_SERVER_BASE_URL'] = undefined;
    });
    ...
    test('env variable', () => {
      process.env['STAINLESS_TOPICLAKE_MCP_SERVER_BASE_URL'] = 'https://example.com/from_env';
      const client = new StainlessTopiclakeMcpServer({ apiKey: 'My API Key' });
      expect(client.baseURL).toEqual('https://example.com/from_env');
    });

    test('empty env variable', () => {
      process.env['STAINLESS_TOPICLAKE_MCP_SERVER_BASE_URL'] = ''; // empty
      const client = new StainlessTopiclakeMcpServer({ apiKey: 'My API Key' });
      expect(client.baseURL).toEqual('https://api.example.com');
    });

    test('blank env variable', () => {
      process.env['STAINLESS_TOPICLAKE_MCP_SERVER_BASE_URL'] = '  '; // blank
      const client = new StainlessTopiclakeMcpServer({ apiKey: 'My API Key' });
      expect(client.baseURL).toEqual('https://api.example.com');
    });
    ...
    test('in request options overridden by env variable', () => {
      process.env['STAINLESS_TOPICLAKE_MCP_SERVER_BASE_URL'] = 'http://localhost:5000/env';
      const client = new StainlessTopiclakeMcpServer({ apiKey: 'My API Key' });
      expect(client.buildURL('/foo', null, 'http://localhost:5000/option')).toEqual(
  ...
  test('maxRetries option is correctly set', () => {
    const client = new StainlessTopiclakeMcpServer({ maxRetries: 4, apiKey: 'My API Key' });
    expect(client.maxRetries).toEqual(4);

    // default
    const client2 = new StainlessTopiclakeMcpServer({ apiKey: 'My API Key' });
  ...
  test('with environment variable arguments', () => {
    ...
    process.env['STAINLESS_TOPICLAKE_MCP_SERVER_API_KEY'] = 'My API Key';
    const client = new StainlessTopiclakeMcpServer();
  ...
  test('with overridden environment variable arguments', () => {
    // set options via env var
    process.env['STAINLESS_TOPICLAKE_MCP_SERVER_API_KEY'] = 'another My API Key';
    const client = new StainlessTopiclakeMcpServer({ apiKey: 'My API Key' });
    expect(client.apiKey).toBe('My API Key');
  });
});
...
describe('retries', () => {
  test('retry on timeout', async () => {
    ...
    const client = new StainlessTopiclakeMcpServer({ apiKey: 'My API Key', timeout: 10, fetch: testFetch });
  ...
  test('retry count header', async () => {
    ...
    const client = new StainlessTopiclakeMcpServer({ apiKey: 'My API Key', fetch: testFetch, maxRetries: 4 });
  ...
  test('retry on 429 with retry-after', async () => {
    ...
    const client = new StainlessTopiclakeMcpServer({ apiKey: 'My API Key', fetch: testFetch });
